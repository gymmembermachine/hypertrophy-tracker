// Hypertrophy Tracker v4 (external JS build)
// Guard: surface early errors
console.log('[HT] app.js loaded');
(function(){
  const root = document.getElementById('tabs');
  const content = document.getElementById('content');
  if(!root || !content){
    console.error('Missing root nodes (#tabs/#content).');
    return;
  }
  ...
  // merge additions
  (function mergeAdd(){ const exists=(bp,ex)=> library.some(([b,e])=> b.toLowerCase()===bp.toLowerCase() && e.toLowerCase()===ex.toLowerCase()); for(const [bp,ex] of additions){ if(!exists(bp,ex)) library.push([bp,ex]); } save(LS_KEYS.library, library) })();
  ...
  function renderTabs(){
    ...
    nav.innerHTML='';
    routes.forEach(r=>{ const b=document.createElement('button'); b.className='tab'+(active===r.id?' active':''); b.textContent=r.label; b.onclick=()=>{active=r.id; render()}; nav.appendChild(b) });
  }
  ...
  function renderLog(){
    ...
    function fillExerciseOptions(){ const bp=bpSel.input.value; const list=bp?exercisesFor(bp):uniq(library.map(x=>x[1])).sort(); exSel.input.innerHTML='<option value=\"\">Select exercise…</option>'+list.map(v=>`<option>${esc(v)}</option>`).join('') }
    ...
    content.append(card, histCard, tblWrap);
    ...
    function renderInlineHistory(){
      ...
      if(best){
        const prText = best.weight>0 ? `PR: ${best.weight} × ${best.reps} (e1RM ${Math.round(best.score)}) on ${fmtDate(best.date)}` : `PR: ${best.reps} reps on ${fmtDate(best.date)}`;
        setPR(prText);
      } else { setPR('No PR yet — log a session to start tracking best set.') }
  ...
  function renderVolume(){
    ...
    card.append(controls, intro, table, chartCard); content.append(card);
  ...
  function renderBackup(){
    ...
    const imp=document.createElement('input'); imp.type='file'; imp.accept='.json'; imp.onchange=(e)=>{ const f=e.target.files[0]; if(!f) return; const rd=new FileReader(); rd.onload=()=>{ try{ const o=JSON.parse(rd.result); if(o.library) library=o.library; if(o.log) log=o.log; localStorage.setItem(LS_KEYS.library, JSON.stringify(library)); localStorage.setItem(LS_KEYS.log, JSON.stringify(log)); alert('Import complete.'); render() }catch(err){ alert('Import failed: '+err.message) } }; rd.readAsText(f) };
    card.append(exp, imp); content.append(card);
  }
